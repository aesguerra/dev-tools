...
/**
  * Created by aesguerra on 1/19/17.
  */
object Job4 extends SparkJob {
  override val name = "Job 4"
  override def run(sc: SparkContext, args: Array[String]): Unit = {
    ...
    rdd.map(m => (m, 1)).reduceByKey(_+_).saveAsTextFile("/home/aesguerra/sample/job4/" + fmt.print(new DateTime))
  }
}
